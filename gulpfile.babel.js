...
var gulpSync = require('run-sequence');
...
gulp.task('compileApplicationScripts', function () {
    if (argv.env === 'release') {
        return browserify(config.root + '/components/metergauge/metergauge.js', {debug: true})
            .transform("babelify",
                {
                    sourceMaps: true,
                    presets: ["es2015"]
...
gulp.task('inject', function () {
    ...
    var vendorCss = gulp.src('./' + config.temp + '/vendor/*.css', {read: true});
    var appCss = gulp.src('./' + config.temp + '/app/*.css', {read: false});


    return gulp.src(config.root + '/index.html')
        .pipe(inject(es.merge(vendorStream, appStream), {name: 'scripts'}))
        .pipe(inject(es.merge(vendorCss, appCss), {name: 'styles'}))
...
gulp.task('injectBuild', function () {
    ...
    return gulp.src(config.root + '/index.html')
        ...
        .pipe(inject(es.merge(appCss, vendorCss), {
